<script lang="ts">
	...
	onMount(() => {
		...
		const storedMotionPreference = JSON.parse(window.localStorage.getItem('reduce-motion'))
	...
	const toggleReducedMotion = (): void => {
		...
		window.localStorage.setItem('reduce-motion', JSON.stringify($prefersReducedMotion))
	}
